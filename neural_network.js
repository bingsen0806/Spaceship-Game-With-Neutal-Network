...
class NeuralNetwork {
  ...
  train(inputArray, targetArray) {
    ...
    // calcultae the deltas (errors * derivitive of the output)
...
function sigmoid(x, deriv = false) {
  if (deriv) {
    return x * (1 - x); //where x = sigmoid(x), the input x is alrdy sigmoid
  }
...
class Matrix {
  constructor(rows, cols, data = []) {
    ...
    //initialise with zeores if no data provided
